  ...
  return <>
    ...
    <div className="evidence-layout">
      <section className="card panel evidence-editor">
        ...
        <div className="actions"><button className="btn btn-secondary" onClick={() => save("draft")}>保存草稿</button><button className="btn btn-primary" onClick={() => save("submit")}>{evaluated.completenessScore === 100 ? "提交为可用证据" : "提交并标记待补事实"}</button></div>{editingId && <Link className="btn btn-link" href={"/expressions?evidenceId=" + editingId}>生成求职表达 →</Link>}
      </section>

...
import { loadProfile, saveAnalysis } from "@/lib/storage.complete";
...
export default function JobsPage() {
  ...
  return <>
    <h1 className="page-title">分析目标 JD</h1><p className="page-subtitle">首版使用本地规则抽取产品/运营能力；每项能力都保留 JD 原文依据。</p>
    <div className="jd-layout">
      <section className="card panel"><div className="card-head"><h2>粘贴岗位描述</h2><button className="btn btn-link" onClick={()=>setJd(exampleJd)}>恢复示例</button></div><textarea className="jd-input" value={jd} onChange={e=>setJd(e.target.value)} aria-label="岗位描述"/><div className="actions"><Link className="btn btn-secondary" href="/onboarding">修改能力画像</Link><button className="btn btn-primary" onClick={analyze} disabled={loading}>{loading?"正在分析…":"分析匹配度 →"}</button></div>{error&&<p className="error">{error}</p>}</section>
      <section className="card panel">
        {!analysis?<div className="empty"><div><div style={{fontSize:42}}>◎</div><h2>等待分析</h2><p>结果将展示岗位能力、你的匹配点、缺口和下一步微项目。</p></div></div>:<div><div className="analysis-summary"><div className="card-head"><h3>{analysis.title}</h3><span className="match-score">{analysis.matchScore}%</span></div><p>{analysis.summary}</p></div><h3>能力要求与原文依据</h3>{analysis.competencies.map(item=><div className="competency-result" key={item.key}><div className="result-top"><div><b>{item.name}</b><div className="muted">你的等级：{item.userLevel}/3 · 差距：{item.gap}</div></div><span className={`badge ${item.gap===0?"good":""}`}>{item.gap===0?"已覆盖":item.importance==="must"?"需优先补":"加分项"}</span></div><p className="quote">“{item.jdQuote}”</p></div>)}<div className="callout"><b>匹配点</b><p>{analysis.strengths.join("；")||"暂无明确证据，请先完善能力画像。"}</p><b>主要缺口</b><p>{analysis.gaps.join("；")||"当前能力已覆盖主要要求。"}</p><b>下一步</b><p>{analysis.nextAction}</p></div><div className="actions"><span></span><Link className="btn btn-primary" href="/plan">生成 7 天微项目 →</Link></div></div>}
      </section>
    </div>
  </>;
}

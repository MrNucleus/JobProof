...
export default function DashboardPage() {
  ...
  const exposure = useMemo(()=>Math.round(profile.competencies.filter(c=>c.level>0).length/profile.competencies.length*100),[profile]);
  const topSkills = [...profile.competencies].sort((a,b)=>(b.level*2+b.evidenceLevel)-(a.level*2+a.evidenceLevel)).slice(0,4);
  const planSectionTitle = currentPlan?.status === "completed" ? "最近完成" : currentPlan?.status === "draft" ? "待开始" : "正在进行";
  const planActionLabel = currentPlan?.status === "completed" ? "查看成果" : currentPlan ? "继续项目" : "创建计划";
  return <>
    ...
    <div className="dash-grid">
      ...
      <section className="card metric-card"><div className="card-head"><h3>已接触能力占比</h3><span className="muted">8 项能力自评</span></div><div className="score-big">{exposure}%</div><div className="meter"><i style={{width:`${exposure}%`}}/></div><p className="muted">补充真实证据，可以提升 JD 匹配可信度。</p></section>
    </div>

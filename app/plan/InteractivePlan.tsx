...
import { JobAnalysis, Plan, Task, UserProfile } from "@/lib/types";
...
function buildPlan(analysis: JobAnalysis, profile: UserProfile): Plan {
  const target = analysis.competencies.find(item => item.gap > 0) || analysis.competencies[0];
  const targetKey = target?.key || "interview";
  const targetName = target?.name || "目标能力";
  const quote = target?.jdQuote || "";
  const now = new Date().toISOString();
  const taskTemplates: Array<Omit<Task, "id">> = [
    { day: "DAY 1", title: "明确问题与访谈对象", instruction: "围绕“" + targetName + "”选择一个真实校园场景，写出目标用户和 3 个待验证假设。", deliverable: "1 页问题定义 + 5 位目标用户名单", estimatedMinutes: 50, status: "todo", competencyKeys: [targetKey], jdQuote: quote },
    { day: "DAY 2", title: "设计验证提纲", instruction: "围绕真实行为设计 6–8 个开放问题，避免诱导式提问，并标注每个问题要验证的假设。", deliverable: "一份可直接使用的访谈提纲", estimatedMinutes: 50, status: "todo", competencyKeys: [targetKey], jdQuote: quote },
    { day: "DAY 3–4", title: "完成 5 次用户访谈", instruction: "记录用户原话、行为、问题和现有替代方案；每次访谈后立即补充事实记录。", deliverable: "5 份原始记录，至少保留 10 条用户原话", estimatedMinutes: 120, status: "todo", competencyKeys: [targetKey, "communication"], jdQuote: quote },
    { day: "DAY 5", title: "整理共性需求", instruction: "对记录做归类，区分高频问题与个别意见，并给出需求优先级。", deliverable: "需求聚类表 + Top 3 问题", estimatedMinutes: 60, status: "todo", competencyKeys: [targetKey, "data"], jdQuote: quote },
    { day: "DAY 6–7", title: "输出结论与建议", instruction: "提出 2 条可执行建议，说明证据、优先级和下一步验证方式。", deliverable: "1 页结论 + 90 秒讲解", estimatedMinutes: 60, status: "todo", competencyKeys: ["communication"], jdQuote: quote }
  ];
  return {
    id: entityId(),
    title: "7 天" + targetName + "验证微项目",
    targetCompetency: targetKey,
    durationDays: 7,
    goal: "针对“" + analysis.title + "”补强" + targetName + "，形成一份可追溯的真实成果。",
    tasks: taskTemplates.map(task => ({ ...task, id: entityId() })),
    createdAt: now,
    updatedAt: now,
    status: "draft",
...
export function InteractivePlan() {
  ...
  function sync() {
    ...
    const currentPlan = existing || buildPlan(currentAnalysis, currentProfile);
    if (!existing) planRepository.save(currentPlan);
    setPlan(currentPlan);
    setSelectedId(currentPlan.tasks[0]?.id || "");

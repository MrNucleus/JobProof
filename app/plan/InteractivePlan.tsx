...
export function InteractivePlan() {
  ...
  return <>
    <div className="sync-banner"><span>✓</span><div><b>已读取最新能力画像</b><small>{profile?.name} · {analysis?.title} · 分析于 {analysis?.analyzedAt ? new Date(analysis.analyzedAt).toLocaleString("zh-CN") : "刚刚"}</small></div><Link href="/onboarding/complete">查看画像</Link></div>

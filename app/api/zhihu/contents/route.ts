...
import { getContents } from "@/lib/zhihu/client";
...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(await getContents(session.accessToken, offsetFrom(url)));
  } catch (error) {
    return NextResponse.json({ message: error instanceof Error ? error.message : "创作列表获取失败" }, { status: 502 });
  }
}

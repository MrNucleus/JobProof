      ...
      <section className="card panel onboarding-single">
        ...
        {visible.length === 0 ? <div className="empty-state"><h3>你还没有选择接触过的能力</h3><p>返回上一步确认至少一项能力，或者继续完成注册，之后用微项目从零开始。</p></div> : visible.map(({ item, index }) => {
          ...
          return <div className="evidence-form" key={item.key}>
            ...
            <span className={`evidence-status ${item.evidenceLevel > 0 && item.evidenceNote.trim() ? "ready" : "pending"}`}>{item.evidenceLevel > 0 && item.evidenceNote.trim() ? "已确认" : "待验证"}</span>
          </div>;
        })}
        <div className="trust-note"><b>JobProof 不替你编经历</b><p>事实不完整时会显示“待补充”，不会自动生成数字或虚构项目结果。</p></div>
        <div className="actions onboarding-actions"><Link className="btn btn-secondary" href="/onboarding/abilities">← 返回</Link><button className="btn btn-primary" type="button" onClick={next}>下一步：查看能力画像 →</button></div>
      </section>
    </>
  );
}
